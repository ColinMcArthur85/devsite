...
(function () {
  ...
  function pad(n) { return String(n).padStart(padWidth, '0'); }
  ...
  function preloadAll() {
    return new Promise((resolve) => {
      ...
      function onLoad() { loaded++; if (loaded >= frameCount) resolve(); }
    });
  }
      ...
      try {
        ...
        function step(now) {
          ...
          if (elapsed >= frameDuration) {
            ...
            idx += 1;
            if (idx >= frameCount) { el.src = staticSrc; resolve(); playingPromise = null; return; }
          }
          rafId = requestAnimationFrame(step);
        }
        rafId = requestAnimationFrame(step);
      } catch (e) { console.error('avatar play error', e); el.src = staticSrc; resolve(); playingPromise = null; }
    });
    return playingPromise;
  }

  function stop() { if (rafId) cancelAnimationFrame(rafId); rafId = null; playingPromise = null; el.src = staticSrc; }
