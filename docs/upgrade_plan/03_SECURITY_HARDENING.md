...
describe("Sanitization Utilities", () => {
  describe("escapeHtml", () => {
    ...
    it("should escape all XSS vectors", () => {
      const vectors = ["<img src=x onerror=alert(1)>", '"><script>alert(1)</script>', "javascript:alert('XSS')", "<svg onload=alert(1)>", '{{constructor.constructor("alert(1)")()}}'];
...
export async function onRequest(context) {
  ...
  const allowedOrigin = env.ALLOWED_ORIGIN || (env.ENVIRONMENT === "production" ? "https://your-production-domain.com" : "http://localhost:5173");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = response.status === 403 ? "API rate limit reached" : "Search service temporarily unavailable";
...
describe("Unsplash API Proxy", () => {
  ...
  describe("Input Validation", () => {
    ...
    it("should sanitize XSS in query parameter", async () => {
      ...
      const request = new Request(`http://localhost/api/unsplash?query=${encodeURIComponent(maliciousQuery)}`);
