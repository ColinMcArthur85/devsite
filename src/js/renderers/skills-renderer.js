...
export function renderSkills() {
  ...
  container.innerHTML = Object.entries(skillsConfig).map(([key, config]) => {
    if (config.isFramework) return ''; // Skip frameworks if they are in a separate section

    const count = skills[key] || 0;
    const percent = count > 0 ? Math.min(100, Math.round((count / 15000) * 100)) : 0; // Assuming 15000 is max for now, or calculate total?
